...
function Pagination($number, $page, $addition){
	...
	if ($number > 1){
		...
		if($page>1){
			echo '<li class="prod-page-item"><a class="page-link" href="?page='.($page-1).$addition.'">Previous</a></li>';
		}
		...
		for($i=0; $i<$number; $i++){
			if(!in_array($i+1,$avaiablePage)){ //nếu không có trong mảng thì ra khỏi vòng for
				if(!$isFirst && $page >3){//nếu chưa có dấu ... và số trang phải lớn hơn 3
					echo'<li class="prod-page-item"><a class="page-link" href="?page='.($page-2).$addition.'">...</a></li>';
