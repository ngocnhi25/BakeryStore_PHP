<?php
function PaginationProduct($number, $page){
	
	if ($number > 1){
		echo '<ul class="prod-page-box">';			
		if($page>1){
			echo '<li class="prod-page-item"><div class="page-link" onclick="pageClickPaginationProduct('.($page-1).')">Previous</div></li>';
		}
		...
		for($i=0; $i<$number; $i++){
			if(!in_array($i+1,$avaiablePage)){ //nếu không có trong mảng thì ra khỏi vòng for
				if(!$isFirst && $page >3){//nếu chưa có dấu ... và số trang phải lớn hơn 3
					echo'<li class="prod-page-item"><div class="page-link" onclick="pageClickPaginationProduct('.($page-2).')">...</div></li>';
					$isFirst = true; //xác nhận đã có dấu ...
				}
				if(!$isLast && $i >$page){
					echo'<li class="page-item"><div class="page-link" onclick="pageClickPaginationProduct('.($page+2).')" >...</div></li>';
			...
			if($page==$i+1){
                ...
                <div class="page-link" onclick="pageClickPaginationProduct('.($i+1).')">'.($i+1).'</div>
                </li>';
			}		
		}
		if($page<$number){
			echo '<li class="prod-page-item">
            <div class="page-link" onclick="pageClickPaginationProduct('.($page+1).')">Next</div></li>';
		}	
