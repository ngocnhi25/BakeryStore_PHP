...
if (isset($_POST['selectedTab'])) {
    ...
    $all = executeResult("SELECT * FROM tb_coupon where CURDATE() BETWEEN start_date AND end_date and qti_coupon > 0 and status = 0");
    $aboutToExprire = executeResult("SELECT * FROM tb_coupon 
                                    WHERE end_date >= '$currentDate' AND end_date <= '$twoDaysLater' and qti_coupon > 0 and status = 0
    ...
    $topUsedCoupon = executeResult("SELECT *, SUM(dc.count_used) as total FROM tb_depot_coupon dc 
                                    ...
                                    WHERE CURDATE() BETWEEN start_date AND end_date and qti_coupon > 0 and status = 0 
    ...
    $couponExpired = executeResult("SELECT * FROM tb_coupon WHERE CURDATE() > end_date or status = 1 ORDER BY coupon_id DESC");
